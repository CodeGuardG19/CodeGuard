...
# ── IAM: save pre-existing LabRole ARN for use by Lambda scripts ──────────────
...
# Lambda is invoked by API Gateway (event-based), no inbound ports needed.
...
# No inbound rules — Lambda is invoked by the API Gateway service principal.
